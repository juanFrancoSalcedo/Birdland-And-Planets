    ...
    internal sealed class Effect
    {
        ...
        private void PrepareScattering(float Scattering, float scale, ref int iterations, ref float spread)
        {
            ...
            */
            ...
            const float maxScattering = 10.0f;
            
            float scaledIterations = scale + Mathf.Min(Scattering, maxScattering) - maxScattering;
            int snappedIterations = Mathf.FloorToInt(scaledIterations);
            iterations = Mathf.Clamp(snappedIterations, 1, 32);
            spread = 0.5f + scaledIterations - snappedIterations;
        ...
        private void UpdateRenderBuffers()
        {
            ...
            float sizeScale = Mathf.Log(Mathf.Max(renderDimension.width, renderDimension.height), 2.0f);
